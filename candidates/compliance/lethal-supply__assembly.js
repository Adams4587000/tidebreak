...
signal.color.setHex(0xff5266);signal.emissive.setHex(0xe43747);// Layered hexagonal pedestal with bracketed polygonal frame.
 add(new THREE.CylinderGeometry(.8,1.1,.3,6),metal,0,.15,0);add(new THREE.CylinderGeometry(.6,.8,.2,6),dark,0,.4,0);
 const pts=[[-1.3,2.8],[1.3,2.8],[0,.55]];for(let i=0;i<3;i++)rod([...pts[i],0],[...pts[(i+1)%3],0],.13,metal);add(new THREE.IcosahedronGeometry(.55,1),signal,0,1.8,0);for(let i=0;i<6;i++){const a=i/6*Math.PI*2;add(new THREE.SphereGeometry(.055,6,4),signal,Math.cos(a)*.75,.48,Math.sin(a)*.75);}
