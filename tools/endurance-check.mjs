...
try{
 ...
 const deadline=Date.now()+460000;let seen=new Set(),result;
 while(Date.now()<deadline){
  const t=await page.evaluate(()=>{const t=window.__GAME__,c=window.testCourse;let offset=0;const s=t.progress*c.length,pick=t.nextPickup;if(pick&&pick.s-s<180&&pick.s>s)offset=pick.offset;const p=c.at(s+Math.max(15,t.speed*.52),offset).p;return{...t,target:Math.atan2(p.x-t.pos[0],p.z-t.pos[1])};});
  ...
  if(t.state==='race'){
   ...
   if(!seen.has(t.sector)&&t.progress>(t.sector+.24)/6){seen.add(t.sector);await page.screenshot({path:`evidence/endurance-sector-${t.sector}.png`});report.sectors.push({sector:t.sector,elapsed:t.elapsed,place:t.place,draws:t.draws,tris:t.tris,pickups:t.pickups});console.log('sector',JSON.stringify(report.sectors.at(-1)));}
  }
  await new Promise(r=>setTimeout(r,65));
 }
 for(const k of [...held])await key(k,false);
 assert.ok(result,'real-input driver finishes');assert.equal(seen.size,6);assert.ok(result.elapsed>240,'endurance duration');assert.equal(result.unlocked[1],true);assert.equal(result.roster[1],true);assert.ok(result.rivals.some(r=>r.claimed>0),'rivals collect shared perks');assert.ok(report.peaks.draws<=900,'draw budget');assert.ok(report.peaks.tris<=1500000,'triangle budget');assert.ok(report.lensMax>0,'water reaches the lens');assert.ok(result.pickups>0,'player can contest supplies');assert.equal(errors.length,0);
 report.result=result;await page.screenshot({path:'evidence/endurance-result.png'});await page.click('#dockb');await page.reload();await page.waitForFunction('window.__READY__');assert.equal(await page.evaluate(()=>window.__GAME__.roster[1]),true);report.saved=await page.evaluate(()=>JSON.parse(localStorage.getItem('tidebreak.endurance.v2')));report.passed=true;console.log('ENDURANCE PASS',JSON.stringify({elapsed:result.elapsed,place:result.place,pickups:result.pickups,peaks:report.peaks,saved:report.saved}));
}catch(e){report.failure=e.stack;process.exitCode=1;console.error(e);}finally{fs.writeFileSync('evidence/endurance-check.json',JSON.stringify(report,null,2));await browser.close();}
