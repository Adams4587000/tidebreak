...
function fleetSurface(m,textures,finish,kind='paint'){
 ...
 m.onBeforeCompile=shader=>{
  ...
  shader.fragmentShader=shader.fragmentShader.replace('#include <map_fragment>',`
   ...
   float hullChip=${coated?`1.-smoothstep(${(.08+finish.wear*.14).toFixed(3)},${(.22+finish.wear*.14).toFixed(3)},hullLuma)`:'0.'};
   ${composite?`diffuseColor.rgb*=mix(.38,1.55,smoothstep(.004,.09,hullLuma));`:
    coated?`diffuseColor.rgb*=mix(.68,1.12,clamp(hullLuma*1.8,0.,1.));diffuseColor.rgb=mix(diffuseColor.rgb,vec3(.15,.18,.20)*(.7+hullRough*.65),hullChip);`:
    `diffuseColor.rgb*=mix(.6,1.2,hullRough);`}
  `);
