export default function generate(THREE){
 ...
 const geo=strategy===0?new THREE.IcosahedronGeometry(1,3):strategy===1?new THREE.DodecahedronGeometry(1,1):new THREE.SphereGeometry(1,13,9);
 const p=geo.attributes.position;for(let i=0;i<p.count;i++){let x=p.getX(i),y=p.getY(i),z=p.getZ(i);const n=1+.13*Math.sin(x*9+z*8)*Math.cos(y*11)+.07*Math.sin(z*17-y*9);p.setXYZ(i,x*n*7,y*n*10,z*n*6);}geo.computeVertexNormals();stone.color.setHex(0x666960);stone.flatShading=false;mesh(geo,stone,0,7,0);
