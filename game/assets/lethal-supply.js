...
signal.color.setHex(0xff5266);signal.emissive.setHex(0xe43747);// Turned stepped plinth and separated floating frame segments.
 lathe([[0,0],[1.1,0],[1.1,.2],[.85,.35],[.85,.45],[.6,.5],[0,.5]],dark);
 const pts=[[-1.25,2.9],[1.25,2.9],[0,.65]];for(let i=0;i<3;i++){const a=pts[i],b=pts[(i+1)%3];tube([[a[0],a[1],0],[(a[0]+b[0])*.5,(a[1]+b[1])*.5,.12],[b[0],b[1],0]],.11,signal);}ball(0,1.85,0,.55,.55,.55,signal);for(const s of [-1,1])box(s*.8,.45,0,.15,.1,.5,signal);

 const ring=new THREE.Group();ring.name='ring';root.children.filter(o=>o.geometry?.type==='TubeGeometry').forEach(o=>ring.add(o));root.add(ring);root.children.find(o=>o.geometry?.type==='SphereGeometry').name='core';
