...
export function buildWorld(course,art){
 ...
 for(let sector=0;sector<6;sector++){
  for(const frac of (course.config.arena===5?[.55]:[.18,.55])){const s=(sector+frac)*course.length/6;if(course.config.arena===1){place('mangrove',s,-course.width-8,2.5,-1);place('wreck',s,course.width+5,.5,-2,.8);continue;}if(course.config.arena===2){place('rock',s,-course.width-12,[2,5,3],-3);place('rock',s,course.width+12,[2,4,3],-3);continue;}structure(s,0,11,course.width*2+8,1.6,8);for(const offset of [-14,14]){structure(s,offset,4,3.6,12,5);obstacles.push({s,offset,radius:3.5,kind:'PIER'});structure(s-3,offset,2.5,4.3,.25,.2,light);}for(const offset of [-course.width,course.width])structure(s,offset,13,.2,3,8,rail);for(const dz of [-3,3])structure(s+dz,0,13,course.width*2, .2,.15,rail);for(let k=-3;k<=3;k++)structure(s,k*8,14,.18,5,.18,rail);}
  for(const frac of [.39,.79]){const s=(sector+frac)*course.length/6,offset=frac<.5?-14:14;if([1,2,3,5].includes(course.config.arena))place(course.config.arena===1||course.config.arena===5?'wreck':'rock',s,offset,course.config.arena===1||course.config.arena===5?[.38,.45,.4]:[.5,.35,.5],-1);else structure(s,offset,.8,12,2,5);obstacles.push({s,offset,radius:7,kind:'BARRIER'});const warning=textPlane(offset<0?'» » KEEP RIGHT » »':'« « KEEP LEFT « «',13,1.5,'#eee6cf','#292d2c');warning.position.copy(course.at(s-2,offset).p);warning.position.y=4;warning.rotation.y=course.at(s).heading+Math.PI;chunks[chunkIndex(s)].add(warning);}
 }
