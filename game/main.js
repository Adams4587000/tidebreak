...
const enableAudio=()=>audio.activate().then(()=>{$('startup-audio').textContent='◖ SOUND READY';}).catch(()=>{});
 document.addEventListener('pointerdown',enableAudio,{capture:true,once:true});document.addEventListener('keydown',enableAudio,{capture:true,once:true});$('startup-audio').onclick=enableAudio;
...
function bindInput(){
 ...
 window.addEventListener('keydown',e=>{if((state==='race'||state==='countdown')&&['ArrowUp','ArrowDown','ArrowLeft','ArrowRight','Space'].includes(e.code))e.preventDefault();if(e.code==='Escape'&&!e.repeat)pause();if(e.code==='KeyR'&&!e.repeat)recover();if(e.code==='KeyF'&&!e.repeat)fireWeapon();if(e.code==='KeyQ'&&!e.repeat)fireWeapon(true);if(e.code==='KeyE'&&!e.repeat)shield();keys.add(e.code);});window.addEventListener('keyup',e=>keys.delete(e.code));window.addEventListener('blur',()=>{clearInput();if(state==='race'||state==='countdown')pause();});document.addEventListener('visibilitychange',()=>{if(document.hidden&&(state==='race'||state==='countdown'))pause();});
 ...
 $('quickraceb').onclick=quickRace;$('enterdock').onclick=()=>{audio.init().catch(()=>{});showSetup('dock');};$('arenasb').onclick=()=>{if(boatUnlocked(save,boatIndex))showSetup('arenas');};$('backdockb').onclick=()=>showSetup('dock');$('homeb').onclick=()=>{revHeld=false;revUntil=0;previewRev=0;state='title';$('menu').hidden=true;$('loading').hidden=false;};
