...
const routes=CHAPTERS.map(makeCourse);
...
function syncArena(){
 ...
 $('course-objective').textContent=available?(mode==='time'?'Solo practice · no territory unlocks':c.objective):'LOCKED · '+CHAPTERS[previewArena-1].objective;
...
function syncMenu(){
 ...
 $('career').textContent=`${save.finishes} FINISHES / ${save.wins} WINS / ${save.reputation||0} REP`;
 syncArena();
}
function showSetup(step){menuStep=step;state='menu';$('loading').hidden=true;$('menu').hidden=false;$('dock-page').hidden=step!=='dock';$('arena-page').hidden=step!=='arenas';$('menu').classList.toggle('choosing-arena',step==='arenas');$('nav-dock').classList.toggle('current',step==='dock');$('nav-arena').classList.toggle('current',step==='arenas');syncMenu();}
async function launchArena(){
 ...
 try{if(chapter!==previewArena){chapter=previewArena;changeCourse();}start();}catch(e){fatal(e);}
}
function quickRace(){
 ...
 if(chapter!==0){chapter=0;changeCourse();}
 start();
}
function disposeWorld(){if(!world)return;scene.remove(world.root);world.dispose();}
function changeCourse(){
 disposeWorld();course=routes[chapter];world=buildWorld(course,art);scene.add(world.root);currentSector=chapter;currentLeg=-1;const c=SECTORS[chapter];ambientAmp=c.waves;scene.fog=new THREE.FogExp2(c.sky,c.fog);scene.background=new THREE.Color(c.sky);water.uniforms.waterColor.value.set(c.water);water.uniforms.deepColor.value.set(c.deep);water.uniforms.skyColor.value.set(c.sky);water.uniforms.amplitude.value=c.waves;water.uniforms.fogDensity.value=c.fog;
...
function finish(eliminated=false){
 ...
 $('result-title').innerHTML=eliminated?'HULL LOST.<br>RUN ENDED.':passed?'TERRITORY<br>CLAIMED.':'FINISH LINE.<br>STILL ALIVE.';
 ...
 $('result-detail').textContent=mode==='time'?'Practice time saved. Career rewards are earned in races.':`${eliminated?'Three hits. Crew evacuated.':passed?'Territory cleared.':'Finish recorded.'} +${(save.reputation||0)-beforeRep} reputation · ${player.kills} takedowns · ${race.pickups} supplies. Crew unlocks and previous records are preserved.`;
...
function updateRivals(dt){if(mode!=='race')return;for(const b of opponents){if(!live(b))continue;tickCombat(b,dt);b.raceTime=elapsed;stepAI(b,BOATS[b.spec],course,world.pickups,dt,elapsed,CHAPTERS[chapter].difficulty,racers());combat.ai(b,racers(),dt,elapsed);}}
...
function toDock(){
 state='menu';lens.clear();combatFX.clear();wasBoosting=false;steering=0;clearInput();
...
function bindInput(){
 ...
 $('quickraceb').onclick=quickRace;$('enterdock').onclick=()=>{audio.init().catch(()=>{});showSetup('dock');};$('arenasb').onclick=()=>{if(boatUnlocked(save,boatIndex))showSetup('arenas');};$('backdockb').onclick=()=>showSetup('dock');$('homeb').onclick=()=>{state='title';$('menu').hidden=true;$('loading').hidden=false;};
 $('inspectb').onclick=()=>{inspectRider=!inspectRider;$('inspectb').textContent=inspectRider?'VIEW MACHINE ↗':'RIDER DETAIL ↗';};$('startb').onclick=launchArena;$('againb').onclick=start;$('restartb').onclick=start;$('dockb').onclick=toDock;$('exitb').onclick=toDock;$('pauseb').onclick=pause;$('resumeb').onclick=pause;$('soundb').onclick=()=>{audio.setEnabled(!audio.enabled);$('soundlabel').textContent=audio.enabled?'ON':'OFF';};
...
async function init(){
 ...
 water=createWater(art.textures['water-normal']);scene.add(water.mesh,wake.mesh,spray.mesh);reflection=createReflection(renderer,water,mobile);renderer.info.autoReset=false;makeHero();setupOpponents();changeCourse();bindInput();state='title';$('loadfill').style.width='100%';renderer.compile(scene,camera);requestAnimationFrame(frame);$('loading-status').hidden=true;$('enterdock').hidden=false;$('quickraceb').hidden=false;$('menu').hidden=true;syncMenu();window.__READY__=true;
}
init().catch(fatal);
