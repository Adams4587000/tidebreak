...
export function makeCourse(config){
 const pts=[];for(let i=0;i<16;i++){const a=i/16*Math.PI*2;const r=config.radius*(1+Math.sin(a*3+config.seed)*config.bend+Math.cos(a*2)*.06);pts.push(new THREE.Vector3(Math.sin(a)*r,0,Math.cos(a)*r*config.stretch));}
 const curve=new THREE.CatmullRomCurve3(pts,true,'centripetal');curve.arcLengthDivisions=1600;const length=curve.getLength(),count=700;
 ...
 return{curve,length,samples,at,nearest,width:config.width,config};
}
