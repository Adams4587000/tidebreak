...
export const PERKS=['CHARGE','SHIELD','OVERDRIVE','REPAIR','AMMO','SEEKER','LASER','MINE','BOMB','DEATH'];
export const PERK_COLORS=[0xe9b967,0x78b7ca,0xf16d43,0x94ac72,0xffad70,0xdf99ff,0x71edff,0xdcd79b,0xff9861,0xff263e];
...
export function stepAI(r,spec,course,pickups,dt,time,difficulty,traffic=[]){
 ...
 for(const p of pickups){const d=p.s-r.s,value=p.type==='CHARGE'&&r.boost<60?32:p.type==='OVERDRIVE'&&r.overdrive<1?30:p.type==='REPAIR'&&r.boost<75?18:0,score=d+Math.abs(p.offset-r.lane)*2-value;if(p.type!=='DEATH'&&!p.collected&&d>5&&d<180&&score<nearest){nearest=score;target=p.offset;}}
 ...
 for(const o of course.obstacles||[]){const d=o.s-r.s;if(d>0&&d<130&&Math.abs(target-o.offset)<o.radius+4)target=o.offset+(r.homeLane>=o.offset?1:-1)*(o.radius+6);}
 ...
 const use=r.launch>0||r.boostLock===0&&r.boost>2&&turn<.23&&(r.boost>55||r.burst>0);
 if(use&&(r.launch||0)<=0){r.burst=Math.max(0,(r.burst||2)-dt);r.boost=Math.max(0,r.boost-dt*22);}else{r.burst=0;r.boost=Math.min(100,r.boost+dt*(10+(r.pursuit||0)*35));}
 r.boosting=use;
 ...
 const targetSpeed=spec.speed*(.945+difficulty*.012+((r.spec||0)%3)*.004)*corner*trafficLoss*pace(r,traffic)*(r.errorLife>0?.93:1)*(use?spec.boost:1)*(r.overdrive>0?1.12:1);
